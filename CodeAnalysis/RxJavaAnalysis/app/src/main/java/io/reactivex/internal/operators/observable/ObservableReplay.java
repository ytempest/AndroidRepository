...
public final class ObservableReplay<T> extends ConnectableObservable<T> implements HasUpstreamObservableSource<T>, Disposable {
    ...
    /** Holds the current subscriber that is, will be or subscriber was subscribed to the source observable. */
     ...
     * @param selector the function that receives an ObservableSource and should return another ObservableSource that will be subscribed to
    ...
    implements Observer<T>, Disposable {
        ...
        boolean add(InnerDisposable<T> producer) {
            ...
            for (;;) {
                ...
                // an onError or onComplete, subscriber refuse to add the new producer
                if (c == TERMINATED) {
                    return false;
                }
        ...
        void remove(InnerDisposable<T> producer) {
            ...
            for (;;) {
                ...
                // we didn't find it so subscriber quit
                if (j < 0) {
                    return;
                }
    ...
    abstract static class BoundedReplayBuffer<T> extends AtomicReference<Node> implements ReplayBuffer<T> {
        ...
        final void removeFirst() {
            ...
            Node next = head.get();
            size--;
            // can't subscriber move the head because it would retain the very first value
            ...
            for (;;) {
                ...
                if (r == null) {
                    ...
                    if (!curr.compareAndSet(null, u)) {
                        ...
                        // to source has subscriber finished
                        continue;
                    }
