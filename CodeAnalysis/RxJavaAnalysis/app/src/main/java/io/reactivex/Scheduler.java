...
public abstract class Scheduler {
    ...
    public Disposable scheduleDirect(@NonNull Runnable run, long delay, @NonNull TimeUnit unit) {
        // createWorker()方法会在指定线程（假设Schedulers.io()）中被实现，在IoScheduler类返回了一个 EventLoopWorker对象
        final Worker w = createWorker();

        // 把执行单元（SubscribeTask）进行装配一下，因为 RxJavaPlugins.onSchedule() 方法中可以
        // 对执行单元设置一层代理，但是默认没有设置
