...
public final class RxJavaPlugins {
    ...
    /**
     * 这个方法会调用 Callable对象中的 call方法，获取其创建的 Scheduler对象（IOScheduler 或
     * HandlerScheduler）,然后做一些封装（如果有需要），最后把这个  Scheduler对象返回
     *
     * @param defaultScheduler Callable对象，其中包含了获取 Scheduler对象的方法
     */
    @NonNull
    public static Scheduler initIoScheduler(@NonNull Callable<Scheduler> defaultScheduler) {
        ...
        if (f == null) {
            // 如果没有设置 onInitIoHandler，那么就会走到这里
            return callRequireNonNull(defaultScheduler);
        }
    ...
    public static Scheduler onIoScheduler(@NonNull Scheduler defaultScheduler) {
        // 判断是否设置了IO线程的拦截器，如果有就使用拦截器代理默认的IO线程
