...
public abstract class Observable<T> implements ObservableSource<T> {
    ...
    /**
     * Provides an API (via a cold ObservableSource) that bridges the reactive world with the callback-style world.
     * <p>
     * Example:
     * <pre><code>
     * ObservableSource.&lt;Event&gt;create(emitter -&gt; {
     ...
     * @param source the emitter that is called when an Observer subscribes to the returned {@code ObservableSource}
     * @return the new ObservableSource instance
    ...
    /**
     * 这个方法会把这个方法之后的所有被观察者对象、观察者对象放到指定的线程中执行
     *
     * @param scheduler 指定的线程
     * @return 返回一个 ObservableObserveOn对象，这个对象中的 subscribeActual() 方法实现了已经把
    ...
    /**
     * 将上一个被观察者对象、线程调度的Scheduler等封装成一个 ObservableObserveOn对象
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.CUSTOM)
    ...
    public final void subscribe(Observer<? super T> observer) {
        ...
        ObjectHelper.requireNonNull(observer, "observer is null");
        try {
            // 如果设置了拦截器，就使用拦截器代理observer
            observer = RxJavaPlugins.onSubscribe(this, observer);

            ObjectHelper.requireNonNull(observer, "Plugin returned null Observer");

            // 这里会执行最后一个map返回的observer 
    ...
    /**
     * 创建一个 ObservableSubscribeOn对象代理这方法之前的被观察者，然后让观察者对象观察这一个代理对象
     * 当这个代理对象被观察者订阅时候（使用 subscribe(ObservableSubscribeOn)方法），那么就会调用这个
     * 代理对象的 subscribeActual() 方法，在这个方法中，就会被代理对象代理的上一个被观察者对象放到指
     * 定的线程（即Scheduler）中执行
     *
     * @param scheduler 被观察者运行的线程
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.CUSTOM)
