...
public abstract class Scheduler {
    ...
    /**
     * 延迟指定的时间后执行 Runnable执行单元；这个方法在多线程调度中是安全，但是它不能保证任务执行
     * 的先后顺序
     *
     * @param run   执行单元
     * @param delay 任务延迟执行的时间
     * @param unit  延迟时间的单位
     * @return the Disposable that let's one cancel this particular delayed task.
     */
    @NonNull
    public Disposable scheduleDirect(@NonNull Runnable run, long delay, @NonNull TimeUnit unit) {
        // createWorker()方法调用 Scheduler的实现类（如：IoScheduler）获取一个 Worker对象
        //假设是 IoScheduler类，那么返回的就是 IoScheduler.EventLoopWorker对象
        final Worker w = createWorker();

        // 交个 RxJavaPlugins进行判断，如果用户还设置了拦截 Runnable执行单元的拦截器，那么就调用
        // 这个拦截器的方法获取新的 Runnable执行单元
        ...
        /**
         * 线程调度的关键！！将任务交给指定的 Worker执行
         * 假设是 IoScheduler，那么就会调用到 EventLoopWorker 的 schedule()方法
         * {@link io.reactivex.internal.schedulers.IoScheduler.EventLoopWorker}
         */
     ...
     * of {@link Completable}s into subscriber one {@link Completable}. There must be
