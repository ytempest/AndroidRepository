...
public final class Schedulers {
    ...
    static {
        ...
        // IO 使用了一种单例模式，保证了 Schedulers中只有一个 IO实例
        // 交个 RxJavaPlugins处理，看用户是否会拦截这个 IoScheduler的实例化过程
    ...
    public static Scheduler io() {
        // 调用 RxJavaPlugins的onIoScheduler()判断有没有设置对创建好的 IoScheduler进行拦截的拦截器，
        // 如果有就使用拦截器返回的 Scheduler
        // IO:默认的IO线程，在Schedulers中的static代码块中被初始化
     ...
     * tasks to the "mainThread" thread. Such behavior requires a blocking-queueing scheduler currently not provided
    ...
    /**
     * Description: 这个类会创建一个 IoScheduler，使用 call() 方法就能获取这个IoScheduler对象；
     * 可以看到这个类使用了一种静态内部类的形式实现了懒加载，防止在不需要的时候就创建 IoScheduler
     * 到时消耗过多的资源
     */
    static final class IOTask implements Callable<Scheduler> {
        @Override
        public Scheduler call() throws Exception {
            // 获取默认的 IoScheduler()对象
