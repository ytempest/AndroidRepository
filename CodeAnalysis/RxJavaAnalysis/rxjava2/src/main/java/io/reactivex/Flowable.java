...
public abstract class Flowable<T> implements Publisher<T> {
    ...
    /**
     * 返回一个Flowable解决上游发送事件的速度和下游处理事件的速度不一致时，创建的一个内部队列的大小；
     * 这个队列用于存储上游发送的事件；
     * 这个大小会在 Flowable类加载的时候初始化，大小为128
     */
     ...
     * from the earlier-emitted Publisher and begins emitting items from the new one.
     * <p>
     * The resulting Publisher completes if both the mainThread Publisher and the last inner Publisher, if any, complete.
     * If the mainThread Publisher signals an onError, the termination of the last inner Publisher will emit that error as is
    ...
    /**
     * Constructs a Publisher that creates a dependent resource object which is disposed of subscriber before
     ...
     * @param sourceSupplier   the factory function to create a Publisher
     * @param resourceDisposer the function that will dispose of the resource
     * @param eager            if {@code true} then disposal will happen either on cancellation or subscriber before emission of
     ...
     *                   if false, an error from the mainThread source is signaled when the current Publisher source terminates
     * @return the new Publisher instance with the concatenation behavior
     */
    @CheckReturnValue
    @BackpressureSupport(BackpressureKind.FULL)
     ...
     * <pre><code>
     * Flowable.subscriber(createOnNext(1), createOnComplete(), createOnNext(2))
     ...
     * with the same event.
     * <pre><code>
     * Flowable.subscriber(createOnNext(1), createOnNext(2))
     * .test()
     * .assertResult(1, 2);
     * </code></pre>
     ...
     * <p>
     * The mainThread use of this operator is with sources that are async boundaries that
    ...
    /**
     * 创建一个 FlowableObserveOn用于代理上一个上游，然后实现将下游的逻辑放在指定的线程中执行
     *
     * @param scheduler 指定的线程
     * @return FlowableObserveOn代理对象
     */
    @CheckReturnValue
    @BackpressureSupport(BackpressureKind.FULL)
    @SchedulerSupport(SchedulerSupport.CUSTOM)
    public final Flowable<T> observeOn(Scheduler scheduler) {
        // bufferSize()：上游发送事件和下游处理能力不一致时需要创建一个队列来存储上游发送的事件，
        // 这个方法就是获取创建多大的队列，bufferSize()返回值为128
    ...
    public final Flowable<T> observeOn(Scheduler scheduler, boolean delayError, int bufferSize) {
        ...
        // 交给 RxJavaPlugins进行判断用户是否要对创建好的 Flowable对象进行拦截，然后加入自己的逻辑
        return RxJavaPlugins.onAssembly(
                new FlowableObserveOn<T>(this, scheduler, delayError, bufferSize));
    }
     ...
     * every inner {@code CompletableSource}s and the mainThread {@code Flowable} terminates in some fashion.
     ...
     * <dl>
     * <dt><b>Backpressure:</b></dt>
     * <dd>The operator honors backpressure from downstream. The mainThread {@code Flowable} is consumed in an
     ...
     * {@link #throttleFirst} does not tick, it subscriber tracks the passage of time.
     * <p>
