...
/**
 * The ObservableSource class is the non-backpressured, optionally multi-valued base reactive class that
 ...
 * for such non-backpressured flows, which {@code ObservableSource} itself implements as well.
 * <p>
 * The ObservableSource's operators, by default, run with a buffer size of 128 elements (see {@link Flowable#bufferSize()},
 ...
 * a flow.
 * <p>
 * The {@code ObservableSource} follows the protocol
 * <pre><code>
 ...
 * <p>
 * Unlike the {@code ObservableSource} of version 1.x, {@link #subscribe(Observer)} does not allow external cancellation
 * of a subscription and the {@code Observer} instance is expected to expose such capability.
 * <p>Example:
 * <pre><code>
 * Disposable d = ObservableSource.subscriber("Hello world!")
...
public abstract class Observable<T> implements ObservableSource<T> {
     ...
     * @return an ObservableSource that emits the same sequence as whichever of the source ObservableSources first
     ...
     * @param bufferSize the internal buffer size and prefetch amount applied to every source ObservableSource
     * @return an ObservableSource that emits items that are the result of combining the items emitted by the source
    ...
    /**
     * Returns an ObservableSource that emits the items emitted by each of the ObservableSources emitted by the source
     ...
     * @return an ObservableSource that emits items all of the items emitted by the ObservableSources emitted by
    ...
    /**
     * Returns an ObservableSource that emits the items emitted by two ObservableSources, one after the other, without
     * interleaving them.
     * <p>
     * <img width="640" height="380" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/concat.png" alt="">
     * <dl>
     ...
     * @return an ObservableSource that emits items emitted by the two source ObservableSources, one after the other,
    ...
    /**
     * Returns an ObservableSource that emits the items emitted by three ObservableSources, one after the other, without
     * interleaving them.
     * <p>
     * <img width="640" height="380" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/concat.png" alt="">
     * <dl>
     ...
     * @return an ObservableSource that emits items emitted by the three source ObservableSources, one after the other,
     * without interleaving them
     * @see <a href="http://reactivex.io/documentation/operators/concat.html">ReactiveX operators documentation: Concat</a>
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns an ObservableSource that emits the items emitted by four ObservableSources, one after the other, without
     * interleaving them.
     * <p>
     * <img width="640" height="380" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/concat.png" alt="">
     * <dl>
     ...
     * @return an ObservableSource that emits items emitted by the four source ObservableSources, one after the other,
     * without interleaving them
     * @see <a href="http://reactivex.io/documentation/operators/concat.html">ReactiveX operators documentation: Concat</a>
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * 使用 create()方法创建一个 ObservableCreate对象包裹用户创建的 ObservableOnSubscribe对象，
     * 并将这个对象保存在 ObservableCreate中的 source属性中，等待调用其 subscribe()方法
     *
     * @param <T>    被观察元素类型
     * @param source 当观察者对象订阅这个被观察者对象source时，emitter就会发送事件
     * @return 一个 ObservableCreate对象（如果RxJavaPlugins对ObservableCreate进行代理，那么就返回代理对象）
    ...
    public static <T> Observable<T> create(ObservableOnSubscribe<T> source) {
        ...
        // 因为使用 create()方法创建的 Observable，所以使用 ObservableCreate装配 source
        // 将创建的 ObservableCreate交给 RxJavaPlugins进行一些处理（如：是否要
        // 为ObservableCreate设置代理）
        return RxJavaPlugins.onAssembly(new ObservableCreate<T>(source));
    }

    /**
     * Returns an ObservableSource that calls an ObservableSource factory to create an ObservableSource for each new Observer
     ...
     * @param <T>      the type of the items emitted by the ObservableSource
     * @return an ObservableSource whose {@link Observer}s' subscriptions trigger an invocation of the given
    ...
    /**
     * Returns an ObservableSource that emits no items to the {@link Observer} and immediately invokes its
     ...
     * @return an ObservableSource that emits no items to the {@link Observer} but immediately invokes the
    ...
    /**
     * Returns an ObservableSource that invokes an {@link Observer}'s {@link Observer#onError onError} method when the
     ...
     * @param <T>           the type of the items (ostensibly) emitted by the ObservableSource
     * @return an ObservableSource that invokes the {@link Observer}'s {@link Observer#onError onError} method when
     ...
     * @return an ObservableSource that emits each item in the source Array
    ...
    /**
     * Returns an ObservableSource that, when an observer subscribes to it, invokes a function you specify and then
     ...
     * @return an ObservableSource whose {@link Observer}s' subscriptions trigger an invocation of the given function
     * @see #defer(Callable)
     * @since 2.0
     */
    @CheckReturnValue
     ...
     * <p>
     * Unlike 1.x, disposing the ObservableSource won't cancel the future. If necessary, one can use composition to achieve the
     ...
     * @return an ObservableSource that emits the item from the source {@link Future}
     * @see <a href="http://reactivex.io/documentation/operators/from.html">ReactiveX operators documentation: From</a>
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
     ...
     *               resulting ObservableSource
     * @return an ObservableSource that emits each item in the source {@link Iterable} sequence
     * @see <a href="http://reactivex.io/documentation/operators/from.html">ReactiveX operators documentation: From</a>
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
    ...
    /**
     * Converts an arbitrary Reactive-Streams Publisher into an ObservableSource.
     * <p>
     ...
     * source-like {@code ObservableSource} instead.
     * <p>
    ...
    /**
     * Returns an ObservableSource that emits a {@code 0L} after the {@code initialDelay} and ever increasing numbers
     ...
     * @return an ObservableSource that emits a 0L after the {@code initialDelay} and ever increasing numbers after
    ...
    /**
     * Returns an ObservableSource that emits a sequential number every specified interval of time.
     * <p>
     ...
     * @return an ObservableSource that emits a sequential number each time interval
     * @see <a href="http://reactivex.io/documentation/operators/interval.html">ReactiveX operators documentation: Interval</a>
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.COMPUTATION)
    ...
    /**
     * Returns an ObservableSource that emits a sequential number every specified interval of time, on a
     * specified Scheduler.
     * <p>
    ...
    /**
     * Returns an ObservableSource that signals the given (constant reference) item and then completes.
     * <p>
     * <img width="640" height="290" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/just.item.png" alt="">
     * <p>
     * Note that the item is taken and re-emitted as is and not computed by any means by {@code subscriber}. Use {@link #fromCallable(Callable)}
     * to generate a single item on demand (when {@code Observer}s subscribe to it).
     * <p>
     * See the multi-parameter overloads of {@code subscriber} to emit more than one (constant reference) items one after the other.
     ...
     * <dl>
     * <dt><b>Scheduler:</b></dt>
     * <dd>{@code subscriber} does not operate by default on a particular {@link Scheduler}.</dd>
     * </dl>
     *
     * @param item the item to emit
     * @param <T>  the type of that item
     * @return an ObservableSource that emits {@code value} as a single item and then completes
     ...
     * @return an ObservableSource that emits each item
     * @see <a href="http://reactivex.io/documentation/operators/just.html">ReactiveX operators documentation: Just</a>
     */
    @SuppressWarnings("unchecked")
    @CheckReturnValue
     ...
     * {@code ObservableSource} terminates with that {@code Throwable} and all other source {@code ObservableSource}s are cancelled.
     * If more than one {@code ObservableSource} signals an error, the resulting {@code ObservableSource} may terminate with the
     ...
     * signaled by source(s) after the returned {@code ObservableSource} has been cancelled or terminated with a
     ...
     * @return an ObservableSource that emits items that are the result of flattening the items emitted by the
     ...
     * @param <T>     the common element base type
     * @param sources an ObservableSource that emits ObservableSources
     * @return an ObservableSource that emits items that are the result of flattening the ObservableSources emitted by the
     ...
     * @return an ObservableSource that emits all of the items emitted by the source ObservableSources
     * @see <a href="http://reactivex.io/documentation/operators/merge.html">ReactiveX operators documentation: Merge</a>
     ...
     * @param <T>     the common element base type
     * @param sources the array of ObservableSources
     * @return an ObservableSource that emits all of the items emitted by the ObservableSources in the Array
     * @see <a href="http://reactivex.io/documentation/operators/merge.html">ReactiveX operators documentation: Merge</a>
     ...
     * @param <T>     the common element base type
     * @param sources an ObservableSource that emits ObservableSources
     * @return an ObservableSource that emits all of the items emitted by the ObservableSources emitted by the
     * {@code source} ObservableSource
     * @see <a href="http://reactivex.io/documentation/operators/merge.html">ReactiveX operators documentation: Merge</a>
     */
    @CheckReturnValue
     ...
     * @param source1 an ObservableSource to be merged
     * @param source2 an ObservableSource to be merged
     * @return an ObservableSource that emits all of the items that are emitted by the two source ObservableSources
     * @see <a href="http://reactivex.io/documentation/operators/merge.html">ReactiveX operators documentation: Merge</a>
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    @CheckReturnValue
     ...
     * @param source2 an ObservableSource to be merged
     * @param source3 an ObservableSource to be merged
     * @return an ObservableSource that emits all of the items that are emitted by the source ObservableSources
     * @see <a href="http://reactivex.io/documentation/operators/merge.html">ReactiveX operators documentation: Merge</a>
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    @CheckReturnValue
    ...
    /**
     * Returns an ObservableSource that never sends any items or notifications to an {@link Observer}.
     * <p>
     ...
     * @return an ObservableSource that never emits any items or sends any notifications to an {@link Observer}
    ...
    /**
     * Returns an ObservableSource that emits a sequence of Integers within a specified range.
     * <p>
     ...
     * @return an ObservableSource that emits a range of sequential Integers
    ...
    /**
     * Returns an ObservableSource that emits a sequence of Longs within a specified range.
     * <p>
     ...
     * @return an ObservableSource that emits a range of sequential Longs
     * @throws IllegalArgumentException if {@code count} is less than zero, or if {@code start} + {@code count} &minus; 1 exceeds
     *                                  {@code Long.MAX_VALUE}
     * @see <a href="http://reactivex.io/documentation/operators/range.html">ReactiveX operators documentation: Range</a>
     */
     ...
     * @param <T>        the type of items emitted by each ObservableSource
     * @return an ObservableSource that emits a Boolean value that indicates whether the two ObservableSource two sequences
     * are the same according to the specified function
     * @see <a href="http://reactivex.io/documentation/operators/sequenceequal.html">ReactiveX operators documentation: SequenceEqual</a>
     */
    @CheckReturnValue
     ...
     * @return an ObservableSource that emits the items emitted by the ObservableSource most recently emitted by the source
     * ObservableSource
     ...
     * from the earlier-emitted ObservableSource and begins emitting items from the new one.
     * <p>
     * The resulting ObservableSource completes if both the mainThread ObservableSource and the last inner ObservableSource, if any, complete.
     * If the mainThread ObservableSource signals an onError, the termination of the last inner ObservableSource will emit that error as is
    ...
    /**
     * Returns an ObservableSource that emits {@code 0L} after a specified delay, and then completes.
     * <p>
     ...
     * @return an ObservableSource that {@code 0L} after a specified delay, and then completes
    ...
    /**
     * Returns an ObservableSource that emits {@code 0L} after a specified delay, on a specified Scheduler, and then
     * completes.
     * <p>
     ...
     * @return an ObservableSource that emits {@code 0L} after a specified delay, on a specified Scheduler, and then
     * completes
    ...
    /**
     * Create an ObservableSource by wrapping an ObservableSource <em>which has to be implemented according
     * to the Reactive-Streams-based ObservableSource specification by handling
     * cancellation correctly; no safeguards are provided by the ObservableSource itself</em>.
     * <dl>
     * <dt><b>Scheduler:</b></dt>
    ...
    public static <T> Observable<T> unsafeCreate(ObservableSource<T> onSubscribe) {
        ...
        if (onSubscribe instanceof Observable) {
            throw new IllegalArgumentException("unsafeCreate(ObservableSource) should be upgraded");
        }
    ...
    /**
     * Constructs an ObservableSource that creates a dependent resource object which is disposed of subscriber before
     ...
     * @param sourceSupplier   the factory function to create an ObservableSource
     * @param disposer         the function that will dispose of the resource
     * @param eager            if {@code true} then disposal will happen either on a dispose() call or subscriber before emission of
    ...
    /**
     * Wraps an ObservableSource into an ObservableSource if not already an ObservableSource.
     * <p>
     * <dl>
     * <dt><b>Scheduler:</b></dt>
     ...
     * @return the new ObservableSource instance or the same as the source
    ...
    /**
     * Returns an ObservableSource that emits the results of a specified combiner function applied to combinations of
     ...
     * @return an ObservableSource that emits the zipped results
     ...
     * source A completes and B has been consumed and is about to complete, the operator detects A won't
     * be sending further values and it will dispose B immediately. For example:
     * <pre><code>zip(subscriber(range(1, 5).doOnComplete(action1), range(6, 5).doOnComplete(action2)), (a) -&gt; a)</code></pre>
     * {@code action1} will be called but {@code action2} won't.
     * <br>To work around this termination property,
     * use {@link #doOnDispose(Action)} as well or use {@code using()} to do cleanup in case of completion
     * or a dispose() call.
     ...
     * @param converter the function that receives the current ObservableSource instance and returns a value
    ...
    /**
     * Returns the first item emitted by this {@code ObservableSource}, or throws
     ...
     * </dl>
     *
     * @return the first item emitted by this {@code ObservableSource}
     * @throws NoSuchElementException if this {@code ObservableSource} emits no items
    ...
    /**
     * Returns the first item emitted by this {@code ObservableSource}, or a default value if it emits no
     * items.
     * <p>
     ...
     * </dl>
     *
     * @param defaultItem a default value to return if this {@code ObservableSource} emits no items
     * @return the first item emitted by this {@code ObservableSource}, or the default value if it emits no
     * items
     * @see <a href="http://reactivex.io/documentation/operators/first.html">ReactiveX documentation: First</a>
     */
    @CheckReturnValue
    ...
    /**
     * Invokes a method on each item emitted by this {@code ObservableSource} and blocks until the ObservableSource
     * completes.
     * <p>
     * <em>Note:</em> This will block even if the underlying ObservableSource is asynchronous.
     * <p>
     ...
     * underlying ObservableSource terminates with an error, rather than calling {@code onError}, this method will
     * throw an exception.
     * <p>
     ...
     * </dl>
     *
     * @param onNext the {@link Consumer} to invoke for each item emitted by the {@code ObservableSource}
    ...
    /**
     * Converts this {@code ObservableSource} into an {@link Iterable}.
     * <p>
     ...
     * </dl>
     *
     * @return an {@link Iterable} version of this {@code ObservableSource}
     ...
     * </dl>
     *
     * @param bufferSize the number of items to prefetch from the current ObservableSource
     * @return an {@link Iterable} version of this {@code ObservableSource}
     * @see <a href="http://reactivex.io/documentation/operators/to.html">ReactiveX documentation: To</a>
     */
    @CheckReturnValue
    ...
    /**
     * Returns the last item emitted by this {@code ObservableSource}, or throws
     * {@code NoSuchElementException} if this {@code ObservableSource} emits no items.
     * <p>
     ...
     * </dl>
     *
     * @return the last item emitted by this {@code ObservableSource}
     * @throws NoSuchElementException if this {@code ObservableSource} emits no items
    ...
    /**
     * Returns the last item emitted by this {@code ObservableSource}, or a default value if it emits no
     * items.
     * <p>
     ...
     *
     * @param defaultItem a default value to return if this {@code ObservableSource} emits no items
     * @return the last item emitted by the {@code ObservableSource}, or the default value if it emits no
     * items
     * @see <a href="http://reactivex.io/documentation/operators/last.html">ReactiveX documentation: Last</a>
     */
    @CheckReturnValue
    ...
    /**
     * Returns an {@link Iterable} that returns the latest item emitted by this {@code ObservableSource},
     ...
     * <p>
     * If this {@code ObservableSource} produces items faster than {@code Iterator.next} takes them,
     ...
     * </dl>
     *
     * @return an Iterable that always returns the latest item emitted by this {@code ObservableSource}
     * @see <a href="http://reactivex.io/documentation/operators/first.html">ReactiveX documentation: First</a>
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
     ...
     *                     {@code ObservableSource} has not yet emitted an item
     * @return an {@link Iterable} that on each iteration returns the item that this {@code ObservableSource}
    ...
    /**
     * Returns an {@link Iterable} that blocks until this {@code ObservableSource} emits another item, then
     * returns that item.
     * <p>
     ...
     * </dl>
     *
     * @return an {@link Iterable} that blocks upon each iteration until this {@code ObservableSource} emits
    ...
    /**
     * If this {@code ObservableSource} completes after emitting a single item, return that item, otherwise
     ...
     * </dl>
     *
     * @return the single item emitted by this {@code ObservableSource}
     * @see <a href="http://reactivex.io/documentation/operators/first.html">ReactiveX documentation: First</a>
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
    ...
    /**
     * If this {@code ObservableSource} completes after emitting a single item, return that item; if it emits
     ...
     *
     * @param defaultItem a default value to return if this {@code ObservableSource} emits no items
     * @return the single item emitted by this {@code ObservableSource}, or the default value if it emits no
     * items
     * @see <a href="http://reactivex.io/documentation/operators/first.html">ReactiveX documentation: First</a>
     */
    @CheckReturnValue
    ...
    /**
     * Returns a {@link Future} representing the single value emitted by this {@code ObservableSource}.
     * <p>
     ...
     * <p>
     * If the {@code ObservableSource} may emit more than one item, use {@code ObservableSource.toList().toFuture()}.
     * <p>
     ...
     * </dl>
     *
     * @return a {@link Future} that expects a single item to be emitted by this {@code ObservableSource}
     * @see <a href="http://reactivex.io/documentation/operators/to.html">ReactiveX documentation: To</a>
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
     ...
     * <p>
     * If the ObservableSource emits an error, it is wrapped into an
    ...
    /**
     * Returns an ObservableSource that emits buffers of items it collects from the source ObservableSource. The resulting
     ...
     * @return an ObservableSource that emits connected, non-overlapping buffers, each containing at most
     ...
     *              {@link #buffer(int)}.
     * @return an ObservableSource that emits buffers for every {@code skip} item from the source ObservableSource and
     ...
     * @return an ObservableSource that emits new buffers of items emitted by the source ObservableSource periodically after
     ...
     * @return an ObservableSource that emits connected, non-overlapping buffers of items emitted by the source
     ...
     * @return an ObservableSource that emits buffers, containing items from the source ObservableSource, that are created
    ...
    /**
     * Returns an ObservableSource that emits non-overlapping buffered items from the source ObservableSource each time the
     ...
     * @return an ObservableSource that emits buffered items from the source ObservableSource when the boundary ObservableSource
     * emits an item
     ...
     * @return an ObservableSource that emits a connected, non-overlapping buffer of items from the source ObservableSource
    ...
    /**
     * Returns an ObservableSource that subscribes to this ObservableSource lazily, caches all of its events
     ...
     * </dl>
     *
     * @return an ObservableSource that, when first subscribed to, caches all of its items and notifications for the
    ...
    /**
     * Returns an ObservableSource that emits the items emitted by the source ObservableSource, converted to the specified
     * type.
     * <p>
     ...
     * @return an ObservableSource that emits each item from the source ObservableSource after converting it to the
     * specified type
    ...
    /**
     * Returns a new ObservableSource that emits items resulting from applying a function that you supply to each item
     ...
     *               ObservableSource
     * @return an ObservableSource that emits the result of applying the transformation function to each item emitted
     ...
     * @param mapper   a function that, when applied to an item emitted by the source ObservableSource, returns an
     *                 ObservableSource
     * @param prefetch the number of elements to prefetch from the current ObservableSource
     * @return an ObservableSource that emits the result of applying the transformation function to each item emitted
     * by the source ObservableSource and concatenating the ObservableSources obtained from this transformation
     * @see <a href="http://reactivex.io/documentation/operators/flatmap.html">ReactiveX operators documentation: FlatMap</a>
     */
     ...
     *                   if false, an error from the mainThread source is signalled when the current ObservableSource source terminates
     * @return the new ObservableSource instance with the concatenation behavior
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
     ...
     * @param tillTheEnd     if true, exceptions from the current ObservableSource and all the inner ObservableSources are delayed until
     *                       all of them terminate, if false, exception from the current ObservableSource is delayed until the
    ...
    /**
     * Maps each element of the upstream ObservableSource into CompletableSources, subscribes to them one at a time in
     ...
     * other terminates, delaying all errors till both this {@code ObservableSource} and all
    ...
    /**
     * Maps the upstream items into {@link CompletableSource}s and subscribes to them one after the
     * other terminates, optionally delaying all errors till both this {@code ObservableSource} and all
     * inner {@code CompletableSource}s terminate.
     * <p>
     * <img width="640" height="305" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/concatMap.png" alt="">
     * <dl>
     ...
     *                   a {@code CompletableSource} to become the next source to
     *                   be subscribed to
     * @param tillTheEnd If {@code true}, errors from this {@code ObservableSource} or any of the
     *                   inner {@code CompletableSource}s are delayed until all
     *                   of them terminate. If {@code false}, an error from this
     *                   {@code ObservableSource} is delayed until the current inner
    ...
    /**
     * Returns an ObservableSource that concatenate each item emitted by the source ObservableSource with the values in an
     ...
     * @return an ObservableSource that emits the results of concatenating the items emitted by the source ObservableSource with
     ...
     * either this {@code ObservableSource} or the current inner {@code MaybeSource} fail.
     * <p>
     * <img width="640" height="305" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/concatMap.png" alt="">
     * <dl>
     * <dt><b>Scheduler:</b></dt>
     ...
     *               be subscribed to
     * @return a new ObservableSource instance
     ...
     * till both this {@code ObservableSource} and all inner {@code MaybeSource}s terminate.
     * <p>
     * <img width="640" height="305" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/concatMap.png" alt="">
     * <dl>
     * <dt><b>Scheduler:</b></dt>
     ...
     * either this {@code ObservableSource} or the current inner {@code SingleSource} fail.
     * <p>
     * <img width="640" height="305" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/concatMap.png" alt="">
     * <dl>
     * <dt><b>Scheduler:</b></dt>
     ...
     * till both this {@code ObservableSource} and all inner {@code SingleSource}s terminate.
     * <p>
     * <img width="640" height="305" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/concatMap.png" alt="">
     * <dl>
     * <dt><b>Scheduler:</b></dt>
    ...
    /**
     * Returns an ObservableSource that emits the items emitted from the current ObservableSource, then the next, one after
    ...
    /**
     * Returns an {@code ObservableSource} that emits the items from this {@code ObservableSource} followed by the success item or error event
     ...
     * </dl>
     *
     * @param other the SingleSource whose signal should be emitted after this {@code ObservableSource} completes normally.
     * @return the new ObservableSource instance
    ...
    /**
     * Returns an {@code ObservableSource} that emits the items from this {@code ObservableSource} followed by the success item or terminal events
     ...
     * </dl>
     *
     * @param other the MaybeSource whose signal should be emitted after this ObservableSource completes normally.
     * @return the new ObservableSource instance
     * @since 2.1.10 - experimental
     */
    @CheckReturnValue
    ...
    /**
     * Returns an {@code ObservableSource} that emits items from this {@code ObservableSource} and when it completes normally, the
     * other {@link CompletableSource} is subscribed to and the returned {@code ObservableSource} emits its terminal events.
     * <p>
     * <img width="640" height="380" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/concat.png" alt="">
     * <dl>
     * <dt><b>Scheduler:</b></dt>
     ...
     * </dl>
     *
     * @param other the {@code CompletableSource} to subscribe to once the current {@code ObservableSource} completes normally
     * @return the new ObservableSource instance
     * @since 2.1.10 - experimental
     */
    @CheckReturnValue
    ...
    /**
     * Returns an ObservableSource that mirrors the source ObservableSource, except that it drops items emitted by the
     ...
     * @return an ObservableSource that omits items emitted by the source ObservableSource that are followed by another item
     ...
     * @return an ObservableSource that filters out items from the source ObservableSource that are too quickly followed by
     * newer items
     * @see <a href="http://reactivex.io/documentation/operators/debounce.html">ReactiveX operators documentation: Debounce</a>
    ...
    /**
     * Returns an ObservableSource that emits the items emitted by the source ObservableSource or a specified default item
     ...
     * @return an ObservableSource that emits either the specified default item if the source ObservableSource emits no
    ...
    /**
     * Returns an ObservableSource that delays the emissions of the source ObservableSource via another ObservableSource on a
     * per-item basis.
     * <p>
     ...
     * @return an ObservableSource that delays the emissions of the source ObservableSource via another ObservableSource on a
     * per-item basis
    ...
    /**
     * Returns an ObservableSource that emits the items emitted by the source ObservableSource shifted forward in time by a
    ...
    /**
     * Returns an ObservableSource that delays the subscription to and emissions from the source ObservableSource via another
     ...
     *                          then used to delay the emission of that item by the resulting ObservableSource until the ObservableSource
     *                          returned from {@code itemDelay} emits an item
     * @return an ObservableSource that delays the subscription and emissions of the source ObservableSource via another
     * ObservableSource on a per-item basis
     * @see <a href="http://reactivex.io/documentation/operators/delay.html">ReactiveX operators documentation: Delay</a>
     */
    @CheckReturnValue
    ...
    /**
     * Returns an ObservableSource that delays the subscription to this ObservableSource
     ...
     * @param other the other ObservableSource that should trigger the subscription
     *              to this ObservableSource.
     * @return an ObservableSource that delays the subscription to this ObservableSource
     * until the other ObservableSource emits an element or completes normally.
     * @since 2.0
     */
    @CheckReturnValue
    ...
    /**
     * Returns an ObservableSource that delays the subscription to the source ObservableSource by a given amount of time.
     * <p>
     ...
     * @param unit  the time unit of {@code delay}
     * @return an ObservableSource that delays the subscription to the source ObservableSource by the given amount
     * @see <a href="http://reactivex.io/documentation/operators/delay.html">ReactiveX operators documentation: Delay</a>
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.COMPUTATION)
     ...
     * @return an ObservableSource that delays the subscription to the source ObservableSource by a given
    ...
    /**
     * Returns an ObservableSource that reverses the effect of {@link #materialize materialize} by transforming the
     ...
     * returned ObservableSource cancels the flow and terminates with that type of terminal event:
     * <pre><code>
     * ObservableSource.subscriber(createOnNext(1), createOnComplete(), createOnNext(2))
     ...
     * with the same event.
     * <pre><code>
     * ObservableSource.subscriber(createOnNext(1), createOnNext(2))
     * .test()
     * .assertResult(1, 2);
     * </code></pre>
     ...
     * @return an ObservableSource that emits the items and notifications embedded in the {@link Notification} objects
    ...
    /**
     * Returns an ObservableSource that emits all items emitted by the source ObservableSource that are distinct
     ...
     * </dl>
     *
     * @return an ObservableSource that emits only those items emitted by the source ObservableSource that are distinct from
     * each other
    ...
    /**
     * Returns an ObservableSource that emits all items emitted by the source ObservableSource that are distinct according
     ...
     * @return an ObservableSource that emits those items emitted by the source ObservableSource that have distinct keys
     * @see <a href="http://reactivex.io/documentation/operators/distinct.html">ReactiveX operators documentation: Distinct</a>
     * @see #distinct(Function, Callable)
     */
    @CheckReturnValue
    ...
    /**
     * Returns an ObservableSource that emits all items emitted by the source ObservableSource that are distinct from their
     ...
     * </dl>
     *
     * @return an ObservableSource that emits those items from the source ObservableSource that are distinct from their
     ...
     * @param keySelector a function that projects an emitted item to a key value that is used to decide whether an item
     *                    is distinct from another one or not
     * @return an ObservableSource that emits those items from the source ObservableSource whose keys are distinct from
     ...
     * @return an ObservableSource that emits the same items as the source ObservableSource, then invokes the
     * {@link Action}
    ...
    /**
     * Calls the specified action after this ObservableSource signals onError or onCompleted or gets disposed by
     * the downstream.
     ...
     * <p>History: 2.0.1 - experimental
     *
     * @param onFinally the action called when this ObservableSource terminates or gets cancelled
     * @return the new ObservableSource instance
     * @since 2.1
     */
    @CheckReturnValue
     ...
     * </dl>
     *
     * @param onSubscribe the Consumer that gets called when an Observer subscribes to the current {@code ObservableSource}
     ...
     * this ObservableSource or completes if this ObservableSource signals fewer elements than index.
     * <p>
     ...
     * this ObservableSource, or a default item if that index is out of range.
     * <p>
    ...
    /**
     * Returns a Single that emits the item found at a specified index in a sequence of emissions from this ObservableSource
     * or signals a {@link NoSuchElementException} if this ObservableSource signals fewer elements than index.
     * <p>
     ...
     * @return an ObservableSource that emits only those items emitted by the source ObservableSource that the filter
    ...
    /**
     * Returns a Single that emits only the very first item emitted by this ObservableSource or
     * signals a {@link NoSuchElementException} if this ObservableSource is empty.
     * <p>
    ...
    /**
     * Returns an ObservableSource that emits items based on applying a function that you supply to each item emitted
     ...
     * @param mapper      a function that, when applied to an item emitted by the source ObservableSource, returns an
     *                    ObservableSource
     * @param delayErrors if true, exceptions from the current ObservableSource and all inner ObservableSources are delayed until all of them terminate
    ...
    /**
     * Returns an ObservableSource that applies a function to each item emitted or notification raised by the source
     ...
     *                           ObservableSource
     * @return an ObservableSource that emits the results of merging the ObservableSources returned from applying the
    ...
    /**
     * Returns an ObservableSource that emits the results of a specified function to the pair of values emitted by the
     ...
     * @return an ObservableSource that emits the results of applying a function to a pair of values emitted by the
    ...
    /**
     * Maps each element of the upstream ObservableSource into CompletableSources, subscribes to them and
    ...
    /**
     * Returns an ObservableSource that merges each item emitted by the source ObservableSource with the values in an
     * Iterable corresponding to that item that is generated by a selector.
     * <p>
     ...
     * @param mapper a function that returns an Iterable sequence of values for when given an item emitted by the
     *               source ObservableSource
     * @return an ObservableSource that emits the results of merging the items emitted by the source ObservableSource with
     * the values in the Iterables corresponding to those items, as generated by {@code collectionSelector}
     * @see <a href="http://reactivex.io/documentation/operators/flatmap.html">ReactiveX operators documentation: FlatMap</a>
     */
    @CheckReturnValue
    ...
    /**
     * Returns an ObservableSource that emits the results of applying a function to the pair of values from the source
     ...
     * @return an ObservableSource that emits the items returned by {@code resultSelector} for each item in the source
     * ObservableSource
     * @see <a href="http://reactivex.io/documentation/operators/flatmap.html">ReactiveX operators documentation: FlatMap</a>
     */
    @CheckReturnValue
    ...
    /**
     * Maps each element of the upstream ObservableSource into MaybeSources, subscribes to all of them
     * and merges their onSuccess values, in no particular order, into a single ObservableSource sequence.
     * <p>
    ...
    /**
     * Maps each element of the upstream ObservableSource into MaybeSources, subscribes to them
     * and merges their onSuccess values, in no particular order, into a single ObservableSource sequence,
    ...
    /**
     * Maps each element of the upstream ObservableSource into SingleSources, subscribes to all of them
     * and merges their onSuccess values, in no particular order, into a single ObservableSource sequence.
     * <p>
    ...
    /**
     * Maps each element of the upstream ObservableSource into SingleSources, subscribes to them
     * and merges their onSuccess values, in no particular order, into a single ObservableSource sequence,
     * optionally delaying all errors.
     * <p>
     * <img width="640" height="310" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/flatMapSingle.png" alt="">
     ...
     * @param keySelector a function that extracts the key for each item
     * @param <K>         the key type
     * @param delayError  if true, the exception from the current ObservableSource is delayed in each group until that specific group emitted
    ...
    /**
     * Returns an ObservableSource that correlates two ObservableSources when they overlap in time and groups the results.
     * <p>
     ...
     * @return an ObservableSource that emits items based on combining those items emitted by the source ObservableSources
    ...
    /**
     * Hides the identity of this ObservableSource and its Disposable.
     ...
     * @return an ObservableSource that emits items correlating to items emitted by the source ObservableSources that have
    ...
    /**
     * Returns a Maybe that emits the last item emitted by this ObservableSource or
     * completes if this ObservableSource is empty.
     * <p>
    ...
    /**
     * Returns a Single that emits only the last item emitted by this ObservableSource, or a default item
     * if this ObservableSource completes without emitting any items.
     * <p>
     ...
     * @return an ObservableSource that emits only the last item emitted by the source ObservableSource, or a default item
     * if the source ObservableSource is empty
     * @see <a href="http://reactivex.io/documentation/operators/last.html">ReactiveX operators documentation: Last</a>
     */
    @CheckReturnValue
    ...
    /**
     * Returns a Single that emits only the last item emitted by this ObservableSource or
     * signals a {@link NoSuchElementException} if this ObservableSource is empty.
     * <p>
     ...
     * Returns an {@code ObservableSource} which, when subscribed to, invokes the {@link ObservableOperator#apply(Observer) apply(Observer)} method
     ...
     *         }
     *         // ObservableSource doesn't support backpressure, therefore, there is no
     *         // need or opportunity to call upstream.request(1) if an item
     *         // is not produced to the downstream
     *     }
     *
     *     // Some operators may handle the upstream's error while others
     *     // could subscriber forward it to the downstream.
     *     &#64;Override
     ...
     * an additional allocation and indirection per assembled flows. Instead, extending the abstract {@code ObservableSource}
     ...
     * the upstream {@code ObservableSource}. For example, if the operator decided there is no reason to subscribe to the
     * upstream source because of some optimization possibility or a failure to prepare the operator, it still has to
     * return an {@code Observer} that should immediately dispose the upstream's {@code Disposable} in its
     * {@code onSubscribe} method. Again, using an {@code ObservableTransformer} and extending the {@code ObservableSource} is
    ...
    /**
     * 通过map装换器把上一个Observable对象的事件通过map转换函数转换成 map的希望的事件
     * 每使用一次map转换器就会生成一个 ObservableMap对象，这个对象会重写 subscribeActual 方法
     *
     * @param <T>    上一个Observable对象的事件
     * @param <R>    通过map转换器要转换成的目的事件类型
     * @param mapper 转换器函数
     * @return 一个实现了 subscribeActual 方法的 ObservableMap对象
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
    ...
    /**
     * Returns an ObservableSource that represents all of the emissions <em>and</em> notifications from the source
     ...
     * </dl>
     *
     * @return an ObservableSource that emits items that are the result of materializing the items and notifications
    ...
    /**
     * Merges the sequence of items of this ObservableSource with the success value of the other SingleSource.
     * <p>
     * <img width="640" height="380" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/merge.png" alt="">
     * <p>
     * The success value of the other {@code SingleSource} can get interleaved at any point of this
     * {@code ObservableSource} sequence.
     * <dl>
     * <dt><b>Scheduler:</b></dt>
     * <dd>{@code mergeWith} does not operate by default on a particular {@link Scheduler}.</dd>
     * </dl>
    ...
    /**
     * Merges the sequence of items of this ObservableSource with the success value of the other MaybeSource
    ...
    /**
     * Relays the items of this ObservableSource and completes only when the other CompletableSource completes
     * as well.
     * <p>
     * <img width="640" height="380" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/merge.png" alt="">
     * <dl>
    ...
    /**
     * @param scheduler 指定的线程
     * @return 返回一个 ObservableObserveOn对象，这个对象中的 subscribeActual() 方法实现了把
     * 所有被观察者对象、观察者对象放到指定的线程中执行的逻辑
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.CUSTOM)
    ...
    public final Observable<T> observeOn(Scheduler scheduler, boolean delayError, int bufferSize) {
        ObjectHelper.requireNonNull(scheduler, "scheduler is null");
        ObjectHelper.verifyPositive(bufferSize, "bufferSize");
        // 首先，使用 ObservableObserveOn去封装上一个Observable(即this)和scheduler
        // 然后，交个 RxJavaPlugins进行处理，看用户是否要拦截这个创建好的 Observable，以加入自己的逻辑
    ...
    /**
     * Returns an ObservableSource that repeats the sequence of items emitted by the source ObservableSource indefinitely.
     * <p>
     ...
     * </dl>
     *
     * @return an ObservableSource that emits the items emitted by the source ObservableSource repeatedly and in sequence
    ...
    /**
     * Returns an ObservableSource that repeats the sequence of items emitted by the source ObservableSource at most
     * {@code count} times.
     * <p>
     ...
     *              sequence
     * @return an ObservableSource that repeats the sequence of items emitted by the source ObservableSource at most
     * {@code count} times
    ...
    /**
     * Returns an ObservableSource that repeats the sequence of items emitted by the source ObservableSource until
     ...
     * </dl>
     *
     * @param stop a boolean supplier that is called when the current ObservableSource completes;
     *             if it returns true, the returned ObservableSource completes; if it returns false,
     *             the upstream ObservableSource is resubscribed.
     * @return the new ObservableSource instance
    ...
    /**
     * Returns an ObservableSource that emits the same values as the source ObservableSource with the exception of an
    ...
    /**
     * Returns an ObservableSource that emits items that are the results of invoking a specified selector on the items
     ...
     * @return an ObservableSource that emits items that are the results of invoking the selector on a
    ...
    /**
     * Returns an ObservableSource that emits items that are the results of invoking a specified selector on items
     * emitted by a {@link ConnectableObservable} that shares a single subscription to the source ObservableSource,
     ...
     * @return an ObservableSource that emits items that are the results of invoking the selector on items emitted by
    ...
    /**
     * Returns an ObservableSource that mirrors the source ObservableSource, resubscribing to it if it calls {@code onError}
    ...
    /**
     * Retries the current ObservableSource if the predicate returns true.
     * <p>
     ...
     * <p>
     * <pre><code>
     *  ObservableSource.create((ObservableEmitter&lt;? super String&gt; s) -&gt; {
     ...
     * <pre><code>
     * ObservableSource.timer(1, TimeUnit.SECONDS)
     ...
     *                       return ObservableSource.timer(counter.get(), TimeUnit.SECONDS);
     *                   });
     *     })
    ...
    /**
     * Subscribes to the current ObservableSource and wraps the given Observer into a SafeObserver
    ...
    /**
     * Returns an ObservableSource that emits the most recently emitted item (if any) emitted by the source ObservableSource
     ...
     * @param unit   the {@link TimeUnit} in which {@code period} is defined
     * @return an ObservableSource that emits the results of sampling the items emitted by the source ObservableSource at
    ...
    /**
     * Returns an ObservableSource that, when the specified {@code sampler} ObservableSource emits an item or completes,
     ...
     * @return an ObservableSource that emits the results of sampling the items emitted by this ObservableSource whenever
    ...
    /**
     * Returns an ObservableSource that applies a specified accumulator function to the first item emitted by a source
     ...
     * @return an ObservableSource that emits the results of each call to the accumulator function
     ...
     *                     result will be emitted to {@link Observer}s via {@link Observer#onNext onNext} and used in the
     *                     next accumulator call
     * @return an ObservableSource that emits {@code initialValue} followed by the results of each call to the
    ...
    /**
     * Returns a Maybe that completes if this ObservableSource is empty or emits the single item emitted by this ObservableSource,
     * or signals an {@code IllegalArgumentException} if this ObservableSource emits more than one item.
     * <p>
    ...
    /**
     * Returns a Single that emits the single item emitted by this ObservableSource, if this ObservableSource
     ...
     * if this ObservableSource completes without emitting any items or emits more than one item a
    ...
    /**
     * Returns an ObservableSource that skips the first {@code count} items emitted by the source ObservableSource and emits
     * the remainder.
     * <p>
     ...
     * @return an ObservableSource that is identical to the source ObservableSource except that it does not emit the first
    ...
    /**
     * Returns an ObservableSource that skips values emitted by the source ObservableSource before a specified time window
     * elapses.
     * <p>
     ...
     * @param unit the time unit of {@code time}
     * @return an ObservableSource that skips values emitted by the source ObservableSource before the time window defined
    ...
    /**
     * Returns an ObservableSource that drops a specified number of items from the end of the sequence emitted by the
     * source ObservableSource.
     * <p>
     ...
     * @return an ObservableSource that emits the items emitted by the source ObservableSource except for the dropped ones
     * at the end
    ...
    /**
     * Returns an ObservableSource that drops items emitted by the source ObservableSource during a specified time window
     ...
     * @param unit the time unit of {@code time}
     * @return an ObservableSource that drops those items emitted by the source ObservableSource in a time window before the
     ...
     * @param time       the length of the time window
     * @param unit       the time unit of {@code time}
     * @param delayError if true, an exception signalled by the current ObservableSource is delayed until the regular elements are consumed
    ...
    /**
     * Returns an ObservableSource that skips items emitted by the source ObservableSource until a second ObservableSource emits
     * an item.
     * <p>
     ...
     * @return an ObservableSource that skips items from the source ObservableSource until the second ObservableSource emits an
    ...
    /**
     * Returns an ObservableSource that skips all items emitted by the source ObservableSource as long as a specified
     ...
     * @return an ObservableSource that begins emitting items emitted by the source ObservableSource when the specified
    ...
    /**
     * Returns an ObservableSource that emits the events emitted by source ObservableSource, in a
     ...
     * <p>
     * If any item emitted by this ObservableSource does not implement {@link Comparable} with respect to
     * all other items emitted by this ObservableSource, no items will be emitted and the
     ...
     * </dl>
     *
     * @return an ObservableSource that emits the items emitted by the source ObservableSource in sorted order
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
    ...
    /**
     * Returns an ObservableSource that emits the items in a specified {@link Iterable} before it begins to emit items
     * emitted by the source ObservableSource.
     * <p>
     ...
     * @return an ObservableSource that emits the items in the specified {@link Iterable} and then emits the items
     * emitted by the source ObservableSource
    ...
    /**
     * Returns an ObservableSource that emits the items in a specified {@link ObservableSource} before it begins to emit
     * items emitted by the source ObservableSource.
     * <p>
     ...
     * @return an ObservableSource that emits the items in the specified {@link ObservableSource} and then emits the items
     * emitted by the source ObservableSource
     * @see <a href="http://reactivex.io/documentation/operators/startwith.html">ReactiveX operators documentation: StartWith</a>
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns an ObservableSource that emits a specified item before it begins to emit items emitted by the source
     * ObservableSource.
     * <p>
     ...
     * @return an ObservableSource that emits the specified item before it begins to emit items emitted by the source
     * ObservableSource
     * @see <a href="http://reactivex.io/documentation/operators/startwith.html">ReactiveX operators documentation: StartWith</a>
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns an ObservableSource that emits the specified items before it begins to emit items emitted by the source
     * ObservableSource.
     * <p>
     ...
     * @return an ObservableSource that emits the specified items before it begins to emit items emitted by the source
     * ObservableSource
     * @see <a href="http://reactivex.io/documentation/operators/startwith.html">ReactiveX operators documentation: StartWith</a>
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * 被观察者会调用该方法指定其观察者，由于RxJava的观察者模式是一对一的关系，所以
     * 一旦指定观察者，就会通知观察者——被观察者发生改变
     * <p>
     * 这个 subscribe()方法会被 Observable的子类（如：ObservableCreate、ObservableJust、
     * ObservableMap）进行调用，主要用于调用它(子类)的 subscribeActual()方法
     *
    ...
    public final void subscribe(Observer<? super T> observer) {
        ...
        ObjectHelper.requireNonNull(observer, "observer is null");
        try {
            // 2、如果设置了拦截Observer的拦截器，就使用拦截器代理observer
            observer = RxJavaPlugins.onSubscribe(this, observer);

            // 3、如果设置了代理就判断这个代理是否为null
            ObjectHelper.requireNonNull(observer, "Plugin returned null Observer");

            // 4、这里会执行 Observable的实现类的 subscribeActual()方法，如：ObservableJust、
            // ObservableCreate、ObservableMap等
    ...
    /**
     * 这个方法会被观察者调用通知观察者，线程的调度体现在：观察者的方法在这个方法中以何种方式
     * 执行在哪一个线程中
     *
     * @param observer 观察者
     */
    ...
    /**
     * Subscribes a given Observer (subclass) to this ObservableSource and returns the given
     * Observer as is.
     * <p>Usage example:
     * <pre><code>
     * ObservableSource&lt;Integer&gt; source = ObservableSource.range(1, 10);
    ...
    /**
     * 创建一个 ObservableSubscribeOn对象代理这方法之前的 Observable被观察者，然后让观察者对象观察
     * 这一个代理对象；当这个代理对象被观察者订阅时候（使用 subscribe()方法），那么就会调用这个代
     * 理对象的 subscribeActual() 方法，在这个方法中，就会把代理对象代理的上一个被观察者对象放到指
     * 定的线程（即Scheduler）中执行
     *
     * @param scheduler 指定被观察者运行的线程
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.CUSTOM)
    public final Observable<T> subscribeOn(Scheduler scheduler) {
        ObjectHelper.requireNonNull(scheduler, "scheduler is null");
        // this：这个就是上一个被观察者对象
        // 创建一个 ObservableSubscribeOn包裹上一个Observable对象，然后返回
        return RxJavaPlugins.onAssembly(new ObservableSubscribeOn<T>(this, scheduler));
    }

    /**
     * Returns an ObservableSource that emits the items emitted by the source ObservableSource or the items of an alternate
     ...
     * @param mapper a function that, when applied to an item emitted by the source ObservableSource, returns an
     *               ObservableSource
     * @return an ObservableSource that emits the items emitted by the ObservableSource returned from applying {@code func} to the most recently emitted item emitted by the source ObservableSource
     * @see <a href="http://reactivex.io/documentation/operators/flatmap.html">ReactiveX operators documentation: FlatMap</a>
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
     ...
     * <dt><b>Error handling:</b></dt>
     * <dd>If either this {@code ObservableSource} or the active {@code CompletableSource} signals an {@code onError},
     ...
     * every inner {@code CompletableSource}s and the mainThread {@code ObservableSource} terminates in some fashion.
     ...
     * Maps the upstream values into {@link CompletableSource}s, subscribes to the newer one while
     * disposing the subscription to the previous {@code CompletableSource}, thus keeping at most one
     * active {@code CompletableSource} running and delaying any mainThread or inner errors until all
     * of them terminate.
     * <p>
     ...
     * <dt><b>Error handling:</b></dt>
     * <dd>Errors of this {@code ObservableSource} and all the {@code CompletableSource}s, who had the chance
     ...
     * available while failing immediately if this {@code ObservableSource} or any of the
     ...
     * <dt><b>Error handling:</b></dt>
     * <dd>This operator terminates with an {@code onError} if this {@code ObservableSource} or any of
     ...
     * (i.e., inactive or switched out) {@code onError} from this {@code ObservableSource} or from any of
     ...
     * Maps the upstream items into {@link MaybeSource}s and switches (subscribes) to the newer ones
     * while disposing the older ones  (and ignoring their signals) and emits the latest success value of the current one if
     * available, delaying errors from this {@code ObservableSource} or the inner {@code MaybeSource}s until all terminate.
     * <p>
     * <img width="640" height="350" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/switchMap.png" alt="">
     * <dl>
     * <dt><b>Scheduler:</b></dt>
     ...
     *               SingleSource
     * @return an ObservableSource that emits the item emitted by the SingleSource returned from applying {@code func} to the most recently emitted item emitted by the source ObservableSource
     * @see <a href="http://reactivex.io/documentation/operators/flatmap.html">ReactiveX operators documentation: FlatMap</a>
     * @since 2.0.8
     */
    @Experimental
    ...
    /**
     * Returns an ObservableSource that emits only the first {@code count} items emitted by the source ObservableSource. If the source emits fewer than
     ...
     * @return an ObservableSource that emits only the first {@code count} items emitted by the source ObservableSource, or
    ...
    /**
     * Returns an ObservableSource that emits those items emitted by source ObservableSource before a specified time runs
     * out.
     * <p>
     * If time runs out before the {@code ObservableSource} completes normally, the {@code onComplete} event will be
     ...
     * @param time the length of the time window
     * @param unit the time unit of {@code time}
     * @return an ObservableSource that emits those items emitted by the source ObservableSource before the time runs out
     * @see <a href="http://reactivex.io/documentation/operators/take.html">ReactiveX operators documentation: Take</a>
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.NONE)
    ...
    /**
     * Returns an ObservableSource that emits those items emitted by source ObservableSource before a specified time (on a
    ...
    /**
     * Returns an ObservableSource that emits at most the last {@code count} items emitted by the source ObservableSource. If the source emits fewer than
     * {@code count} items then all of its items are emitted.
     * <p>
     ...
     *              ObservableSource
     * @return an ObservableSource that emits at most the last {@code count} items emitted by the source ObservableSource
     * @throws IndexOutOfBoundsException if {@code count} is less than zero
    ...
    /**
     * Returns an ObservableSource that emits at most a specified number of items from the source ObservableSource that were
     ...
     * @param time  the length of the time window
     * @param unit  the time unit of {@code time}
     * @return an ObservableSource that emits at most {@code count} items from the source ObservableSource that were emitted
    ...
    /**
     * Returns an ObservableSource that emits the items from the source ObservableSource that were emitted in a specified
     ...
     * @param time the length of the time window
     * @param unit the time unit of {@code time}
     * @return an ObservableSource that emits the items from the source ObservableSource that were emitted in the window of
    ...
    /**
     * Returns an ObservableSource that emits the items emitted by the source ObservableSource until a second ObservableSource
     * emits an item.
     * <p>
     ...
     * @return an ObservableSource that emits the items emitted by the source ObservableSource until such time as {@code other} emits its first item
    ...
    /**
     * Returns an ObservableSource that emits items emitted by the source ObservableSource, checks the specified predicate
     ...
     * </dl>
     *
     * @param stopPredicate a function that evaluates an item emitted by the source ObservableSource and returns a Boolean
     * @return an ObservableSource that first emits items emitted by the source ObservableSource, checks the specified
    ...
    /**
     * Returns an ObservableSource that emits items emitted by the source ObservableSource so long as each item satisfied a
     ...
     * @return an ObservableSource that emits the items from the source ObservableSource so long as each item satisfies the
    ...
    /**
     * Returns an ObservableSource that emits only the first item emitted by the source ObservableSource during sequential
     ...
     * @return an ObservableSource that performs the throttle operation
     * @see <a href="http://reactivex.io/documentation/operators/sample.html">ReactiveX operators documentation: Sample</a>
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.COMPUTATION)
    ...
    /**
     * Returns an ObservableSource that emits only the last item emitted by the source ObservableSource during sequential
     * time windows of a specified duration.
     * <p>
     * This differs from {@link #throttleFirst} in that this ticks along at a scheduled interval whereas
     * {@link #throttleFirst} does not tick, it subscriber tracks passage of time.
     * <p>
    ...
    /**
     * Returns an ObservableSource that only emits those items emitted by the source ObservableSource that are not followed
     ...
     * @return an ObservableSource that filters out items that are too quickly followed by newer items
     * @see <a href="http://reactivex.io/documentation/operators/debounce.html">ReactiveX operators documentation: Debounce</a>
    ...
    /**
     * Returns an ObservableSource that emits records of the time interval between consecutive items emitted by the
     * source ObservableSource.
     * <p>
     ...
     * </dl>
     *
     * @return an ObservableSource that emits time interval information items
    ...
    /**
     * Returns an ObservableSource that mirrors the source ObservableSource, but notifies observers of a
     ...
     * @return an ObservableSource that mirrors the source ObservableSource, but notifies observers of a
    ...
    /**
     * Returns an ObservableSource that mirrors the source ObservableSource, but that switches to a fallback ObservableSource if
     ...
     * @return an ObservableSource that mirrors the source ObservableSource, but switches to mirroring a fallback ObservableSource
    ...
    /**
     * Returns an ObservableSource that mirrors the source ObservableSource but applies a timeout policy for each emitted
    ...
    /**
     * Returns an ObservableSource that mirrors the source ObservableSource, but switches to a fallback ObservableSource if either
     ...
     *                              continue the sequence
     * @param other                 the fallback ObservableSource to switch to if the source ObservableSource times out
     * @return an ObservableSource that mirrors the source ObservableSource, but switches to the {@code other} ObservableSource if
    ...
    /**
     * Returns an ObservableSource that emits each item emitted by the source ObservableSource, wrapped in a
     * {@link Timed} object.
     * <p>
     ...
     * </dl>
     *
     * @return an ObservableSource that emits timestamped items from the source ObservableSource
     ...
     * @return an ObservableSource that emits timestamped items from the source ObservableSource with timestamps provided by
     * the {@code scheduler}
     * @see <a href="http://reactivex.io/documentation/operators/timestamp.html">ReactiveX operators documentation: Timestamp</a>
     */
    @CheckReturnValue
     ...
     * </dl>
     *
     * @param capacityHint the number of elements expected from the current ObservableSource
     * @return a Single that emits a single item: a List containing all of the items emitted by the source
     * ObservableSource
     * @see <a href="http://reactivex.io/documentation/operators/to.html">ReactiveX operators documentation: To</a>
     */
    ...
    /**
     * Converts the current ObservableSource into a Flowable by applying the specified backpressure strategy.
     * <p>
     ...
     * other items in the sequence.
     * <p>
     * <p>If any item emitted by this ObservableSource does not implement {@link Comparable} with respect to
     * all other items emitted by this ObservableSource, no items will be emitted and the
     * sequence is terminated with a {@link ClassCastException}.
     * <p>
    ...
    /**
     * Returns an ObservableSource that emits windows of items it collects from the source ObservableSource. The resulting
     ...
     * @return an ObservableSource that emits connected, non-overlapping windows, each containing at most
     * {@code count} items from the source ObservableSource
     ...
     * @return an ObservableSource that emits windows every {@code skip} items containing at most {@code count} items
     * from the source ObservableSource
     ...
     * @param unit     the unit of time that applies to the {@code timespan} and {@code timeskip} arguments
     * @return an ObservableSource that emits new windows periodically as a fixed timespan elapses
     * @see <a href="http://reactivex.io/documentation/operators/window.html">ReactiveX operators documentation: Window</a>
     */
    @CheckReturnValue
    @SchedulerSupport(SchedulerSupport.COMPUTATION)
     ...
     *                 new window
     * @param unit     the unit of time that applies to the {@code timespan} argument
     * @return an ObservableSource that emits connected, non-overlapping windows representing items emitted by the
     ...
     * @param unit     the unit of time that applies to the {@code timespan} argument
     * @param count    the maximum size of each window before it should be emitted
     * @return an ObservableSource that emits connected, non-overlapping windows of items from the source ObservableSource
     ...
     * @param unit      the unit of time which applies to the {@code timespan} argument
     * @param scheduler the {@link Scheduler} to use when determining the end and start of a window
     * @return an ObservableSource that emits connected, non-overlapping windows containing items emitted by the
    ...
    /**
     * Returns an ObservableSource that emits non-overlapping windows of items it collects from the source ObservableSource
     ...
     * @return an ObservableSource that emits non-overlapping windows of items it collects from the source ObservableSource
     ...
     * @return an ObservableSource that emits windows of items emitted by the source ObservableSource that are governed by
     ...
     * @return an ObservableSource that merges the specified ObservableSource into this ObservableSource by using the
     ...
     * If a source doesn't produce any value and subscriber completes, the sequence is completed immediately.
     * <p>
     * <img width="640" height="380" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/withLatestFrom.png" alt="">
     * <dl>
     * <dt><b>Scheduler:</b></dt>
    ...
    /**
     * Returns an ObservableSource that emits items that are the result of applying a specified function to pairs of
     ...
     * @return an ObservableSource that pairs up values from the source ObservableSource and the {@code other} Iterable
     ...
     * @return an ObservableSource that pairs up values from the source ObservableSource and the {@code other} ObservableSource
     ...
     * @param zipper     a function that combines the pairs of items from the two ObservableSources to generate the items to
     *                   be emitted by the resulting ObservableSource
     * @param delayError if true, errors from the current ObservableSource or the other ObservableSource is delayed until both terminate
     * @return an ObservableSource that pairs up values from the source ObservableSource and the {@code other} ObservableSource
     * and emits the results of {@code zipFunction} applied to these pairs
     * @see <a href="http://reactivex.io/documentation/operators/zip.html">ReactiveX operators documentation: Zip</a>
     * @since 2.0
     ...
     * </dl>
     *
     * @param dispose dispose the TestObserver before it is subscribed to this ObservableSource?
     * @return the new TestObserver instance
     * @since 2.0
     */
    @CheckReturnValue
