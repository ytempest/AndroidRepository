 ...
 * dispose action} is executed on the mainThread thread. When unsubscription occurs on a different
 * thread than the mainThread thread, the action is posted to run on the mainThread thread as soon as possible.
 * <p>
 * Instances of this class are useful in creating observables which interact with APIs that can
 * only be used on the mainThread thread, such as UI objects.
 * <p>
 * A {@link #verifyMainThread() convenience method} is also provided for validating whether code
 * is being called on the mainThread thread. Calls to this method along with instances of this class are
...
public abstract class MainThreadDisposable implements Disposable {
    /**
     * Verify that the calling thread is the Android mainThread thread.
     * <p>
    ...
    public static void verifyMainThread() {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            throw new IllegalStateException(
                "Expected to be called on the mainThread thread but was " + Thread.currentThread().getName());
        }
    }
