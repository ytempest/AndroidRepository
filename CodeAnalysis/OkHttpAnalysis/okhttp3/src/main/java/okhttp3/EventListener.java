...
public abstract class EventListener {
     ...
     * 这个方法会在 RealCall中的 enqueue进行调用，也就是开始进行请求的时候进行调用
     * <p>
     ...
     * 这个方法会在进行Socket网络连接的时候调用(在连接之前调用，但是下一刻就开始连接)
     * 待检验：这个方法有可能不会被调用，那就是只有在创建了一个新的RealConnection连接的时候才会
     * 进行Socket连接，如果从StreamAllocation中已经存在一个连接，或者可以从连接池中获取到该连接，
     * 那么就会使用该连接，而不会创建新的连接，也就是说不会进行Socket连接
     * <p>
     ...
     * 这个方法会在Socket连接成功后调用
     * 可能不会调用该方法的原因和connectStart()方法一样，待检验
     * <p>
     ...
     * 当Socket连接出错的时候会调用
     * 但是，如果route路由是可用的以及该错误可以处理进行恢复的话，那么这个就不会是最终的错误
     * <p>
     ...
     * 这个方法会在成功获取到一个RealConnection后调用，这个RealConnection已经连接了服务器；
     * 这个RealConnection的来源：
     * （1）可能是StreamAllocation保存的连接
     * （2）可能是RealConnection连接池中的一个连接
     * （3）可能是新创建的一个新连接
     * <p>
     * <p>This can be invoked more than 1 time for a single {@link Call}. For example, if the response
     ...
     * 该方法会在开始将Request的RequestBody写到服务器的输出流时调用，如果Request没有RequestBody，那么该方法就不会被调用
     * <p>
     * <p>The connection is implicit, and will generally relate to the last
     * {@link #connectionAcquired(Call, Connection)} event.
     * <p>
     ...
     * 当完成了将Request的RequestBody写入到服务器的输出流时，这个方法会被调用；如果Request没有RequestBody，则不会被调用
     * <p>
     ...
     * 当Response开始接收服务器的Response的响应头信息的时候会调用该方法
     * <p>
     * <p>The connection is implicit, and will generally relate to the last
     * {@link #connectionAcquired(Call, Connection)} event.
     * <p>
     ...
     * 当OkHttp完成接收服务器返回的响应头后会立即调用这个方法
     * <p>
