...
public final class MultipartBody extends RequestBody {
     ...
     *
     * @param sink       服务器的输出流
     * @param countBytes 如果为true则只计算表单的长度，不写到服务器的输出流
     * @return 返回表单的长度（单位：byte）
     */
    private long writeOrCountBytes(
        ...
        for (int p = 0, partCount = parts.size(); p < partCount; p++) {
            ...
            if (countBytes) {
                ...
                // 调用表单键值对的值的封装对象（RequestBody）的方法，通过这个方法实现将
                // 上传的文件写到服务器中，可以发现文件写到服务器的逻辑是由上传文件自身实现的
                body.writeTo(sink);
            }
